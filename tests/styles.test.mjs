...
test('all style definitions and five-level numbering retain the pinned Bruce defaults', () => {
  assert.deepEqual(createStyles(), reference.styles);
...
test('DOCX styles XML and rendered paragraph formatting match the reference converter', async () => {
  const zip = await build();
  assert.equal(await zip.file('word/styles.xml').async('string'), await readFile(new URL('../fixtures/reference/styles.xml', import.meta.url), 'utf8'));
  ...
  try {
    ...
    actualParagraphs.forEach((paragraph, i) => assert.equal(paragraph.outerHTML, expectedParagraphs[i].outerHTML, `paragraph ${i}`));
...
test('images use the original 560px display width, aspect ratio and block spacing', async () => {
  ...
  try {
    ...
    assert.equal(extent.getAttribute('cx'), String(560 * 9525));
    assert.equal(extent.getAttribute('cy'), String(560 * 9525));
