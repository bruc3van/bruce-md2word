...
原始 HTML 按文本保留。脚注定义与引用保留为普通文本，并报告 `FOOTNOTE_NOT_CONVERTED`；严格模式拒绝这类降级。任务复选框仍按普通文本输出。

正文使用首行两字符的独立样式；列表换行与续段对齐正文，嵌套代码、图片和表格按容器宽度排版。独立图片居中、不带首行缩进，小图默认不放大；表格保留 Markdown 指定的左、中、右对齐。
