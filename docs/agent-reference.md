...
脚注当前保留为文本并产生 `FOOTNOTE_NOT_CONVERTED`，不是原生 Word 脚注。无法解析到标题书签的文内链接产生 `LINK_UNAVAILABLE`。

编号仍遵循 Markdown：每个独立列表使用其首项起点，跨章节的新列表独立编号；子列表结束后父列表续号。连续列表后续项手写的数字不作为强制跳号。此次不增加跨列表自动续接或章节标题自动编号。
